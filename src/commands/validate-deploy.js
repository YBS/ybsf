...
async function runValidateDeploy({
  ...
  try {
    ...
    } else if (skipDestructive && applyDestructive) {
      prepared.warnings.push("Ignoring --apply-destructive because --skip-destructive was set.");
    }
    ...
    if (deployId) {
      ...
      const reportResult = await runSfCommand({
        ...
        cwd: process.cwd(),
        artifactsDir: prepared.runDir,
        artifactBaseName: "project-validate-deploy-report",
        streamLiveOutput: false,
      });
    ...
    return {
      ...
      destructiveSkipped: prepared.destructiveSkipped,
